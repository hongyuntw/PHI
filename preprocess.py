...
def number_normalize(article):
    article = article.replace('1', '一').replace('.', '點').replace('2', '二') \
        .replace('3', '三').replace('4', '四').replace('5', '五').replace('6', '六') \
            .replace('7', '七').replace('8', '八').replace('9', '九').replace('0', '零')
    return article
...
with open ('./dataset/' + args.file_name + '.json', 'r') as json_file:
    ...
    for data in tqdm(data_file):
        ...
        article = number_normalize(article)
        
        tokens = tokenizer.tokenize(article)
        if (args.mode == "train"):
            start_pos_labels = np.full(len(tokens), 0)
            end_pos_labels = np.full(len(tokens), 0)
            ...
            for i in range(len(tokens)):
                if (tokens[i] == '_'):
                    ...
                    if (count_back == 0):
                        begin = i+1
                        count_back += 1
                        start_pos_labels[i+1] = 1
                    else:
                        end_pos_labels[i-1] = 1
                        type_label[begin : i] = type_list[j]
                        j += 1
                        count_back = 0

            start_pos_labels = start_pos_labels.tolist()
            end_pos_labels = end_pos_labels.tolist()
            type_label = type_label.tolist()

            for i in sorted(remove_list, reverse=True):
                del tokens[i], end_pos_labels[i] , start_pos_labels[i],  type_label[i]
        
        # tokens[0] = "[CLS]"
        if (tokens[0] == "[SEP]"):
            del tokens[0]
            if (args.mode == "train"):
                del start_pos_labels[0], end_pos_labels[0] ,  type_label[0]
        tokens.append("[SEP]")
        if (args.mode == "train"):
            start_pos_labels.append(0)
            end_pos_labels.append(0)
            type_label.append(0)

        ids = tokenizer.convert_tokens_to_ids(tokens)
        # print(tokens)
        # print(start_pos_labels)
        # print(end_pos_labels)
        # flag = False
        # for i in range(len(tokens)):
        #     if start_pos_labels[i] == 1:
        #         flag = True
        #     if flag:
        #         print(tokens[i] , end='')
        #     if end_pos_labels[i] == 1:
        #         flag = False
        #         print('--------')
        # input()
        ...
        if (args.mode == "train"):
            pt_dict = {'input_ids': ids,
                        "start_pos_label": start_pos_labels,
                        "end_pos_label": end_pos_labels,
...
for data in tqdm(bert_data):
    ...
    ids = data['input_ids']
    if (args.mode == "train"):
        start = data['start_pos_label']
        end = data['end_pos_label']
    ...
    while (pos < len(ids)):
        ...
        if (args.mode == "train"):
            start_512 = [0] + start[pos: pos + sep_pos + 1] + [0] * (512 - sep_pos - 2)
            end_512 = [0] + end[pos : pos+sep_pos+1] + [0] * (512 - sep_pos - 2)
        ...
        if (args.mode == "train"):
            pt_dict = {"input_ids":ids_512, "seg":seg_512, "att":att_512,
                        "start_pos_label": start_512, "type_label": type_512,
                        "end_pos_label" : end_512,
...
print('train data size :' , len(bert_data_train_512))
print('test data size : ', len(bert_data_test_512))
